...
```
docker run ubunut:latest
```
But there's an issue, **docker will run the container as long as there is an active process running in it. Ubuntu machine has none. Hence it imediately stopped after getting started.**

**To make it run 24/7 we need to run it in detached mode (run in the background with full control)**. For this you just `-d` flag in the command 

```
docker run -d ubuntu:latest
```
Here we can give many other flags as per our use, like `--name <some_name>` to give name to your container 
`-p <machine_port:container_port>` to open/set-up a port for yur container.


## SSH (Secure Shell)
**It is a protocol used to access remote computers.** Here you will be performing SSH into your container to get the access of and perform various activities.

By default SSH runs on port `22`, hence when creating a container we open up a port that attach to this specific port for SSH access.
...
Now once everything is setup, we can now try entering our container with the following command 
...
But here we don't get Git (we get it by default in linux machines btw), but here we don't, hence we need to install it. 
...
**tbh in this demo, we can't make people access our site but atleast fix our URL somehow**

If you have notice, websites have URLs like `https://google.com`, `https://zed.dev/` etc.
...
You can check this by going to the network tab of your page by inspecting it. The below photo shows the the port `https://google.com` hits when you visit it. 
...
Our site is currently running on `HTTP` and on port `4321`, and we know that `HTTP` or we can say `localhost` runs by default on port `80`. So even you if you type `localhost` on the browser without the port it will open up a page and is a valid address. 
...
When running the command above instead of port 4321 we can give bind the container to port 80. Carefully see the `-p` flag.
...
**But here come another issue.** What happens if there are multiple servers inside our container. We can't bind each one of them to port `80` as it can be used by one at a time, and obviously you'll not buy or in this case run another machine just to run another server and bind it port 80.

![Diagram](./fourth.png)

## Introducing reverse proxy

What if we let our applications/servers running on the port they are and there's another service/system that sits just before our running applications. It's only work is to **intercept the incoming requests, check the domain from which they're from and redirect to the respective ports.** This is called reverse proxy.
...
It listens for the incoming requests on port `80`, and check the domain it is coming from. If it is `localhost`, iot redirects the request to port `4321` on which our application is running.
...
Save this config and reload the ngnix server.
...
## Issue 2
The browser identify our site as not secure. You may check the status yourself from the browsrer itself as shown below in the photo.
...
If you remember we've already discussed about the default port of `HTTP` and `HTTPS`. Currrently the requests coming on port `80` is captured by nginx and redirected to our application running on port `4321`. 
...
> We use TLS in today's time and not SSL, but the industry call them as SSL certificates out of habit.


So the next step to solve the issue is to generate the certifcate for our site and give to nginx, so it can handle all the verification and other process as all the request is handled by nginx before it reaches our actual server.

There are many services paid and unpaid using whch you can generate the certificates. I'm gonna use one of them known as `mkcert`.
...
It will automatically update our `ngninx.conf` file referencing these certificates.
...
## Conclusion
Starting from running an ubuntu machine on our system to run our  application on HTTPS, we've covered seevral topics including reverse proxy and SSH. For issue 3 we need to buy a domain, for now we can think `localhost` as our domain.
