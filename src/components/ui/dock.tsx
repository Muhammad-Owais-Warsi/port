...
export default function Dock() {
    const links = [
        ...
        },
        {
            ...
            icon: <SiPeerlist className="w-4 h-4 sm:w-5 sm:h-5" />,
        },
    ];

    return (
        <TooltipProvider>
            {/* 1. Use bg-background/80 for the glass effect
                2. Use border-border for standard shadcn borders
            */}
            <div className="fixed bottom-2 sm:bottom-6 left-1/2 transform -translate-x-1/2 z-50">
                <div className="flex items-center gap-1 sm:gap-2 rounded-full border border-border bg-background px-3 sm:px-4 py-2 sm:py-3">
                        ...
                        <Tooltip key={index}>
                            <TooltipTrigger asChild>
                                <a
                                    ...
                                    className="flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 rounded-full text-muted-foreground hover:text-primary"
                                >
                                    {link.icon}
